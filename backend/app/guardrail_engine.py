...
RULES: list[DetectionRule] = [
    ...
    DetectionRule("API key disclosure", "AI Provider Secret", re.compile(r"(?i)\b(?:[a-z0-9_.-]+\s+){0,4}(api[_\s-]?key|access[_\s-]?key|client[_\s-]?secret|secret[_\s-]?key)\s*(is|=|:)\s*['\"]?[A-Za-z0-9_.\-]{3,}"), "Critical", "block", 0.94),
    ...
    DetectionRule("Password assignment", "Credential", re.compile(r"(?i)\b(?:[a-z0-9_.-]+\s+){0,4}(password|passwd|pwd)\s*[:=]\s*['\"]?[^'\"\s]{3,}"), "High", "block", 0.9),
    DetectionRule("Password disclosure", "Credential", re.compile(r"(?i)\b(?:[a-z0-9_.-]+\s+){0,4}(password|passwd|pwd)\s+(is|=|:)\s*['\"]?[^'\"\s]{3,}"), "High", "block", 0.92),
    DetectionRule("Token disclosure", "Access Token", re.compile(r"(?i)\b(?:[a-z0-9_.-]+\s+){0,4}(token|auth[_\s-]?token|bearer[_\s-]?token)\s*(is|=|:)\s*['\"]?[A-Za-z0-9_.\-]{3,}"), "Critical", "block", 0.92),
